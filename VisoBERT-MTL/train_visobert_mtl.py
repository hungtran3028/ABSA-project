...
def save_mtl_predictions(metrics: dict, aspect_names: list, output_dir: str):
    ...
    ad_preds = metrics['ad']['predictions']  # numpy [N, 11]
    ad_labels = metrics['ad']['labels']      # numpy [N, 11]
    sc_preds = metrics['sc']['predictions']  # tensor [N, 11]
    sc_labels = metrics['sc']['labels']      # tensor [N, 11]
    ...
    for i in range(n_samples):
        ...
        for j, aspect in enumerate(aspect_names):
            ...
            # SC correct only evaluated where aspect is present
            if al == 1:
