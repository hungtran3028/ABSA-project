...
class ErrorAnalyzer:
    ...
    def __init__(self, test_file=None, predictions_file=None):
        """
        Khởi tạo ErrorAnalyzer
        
        Args:
            test_file: File CSV chứa ground truth (None = auto-detect)
            predictions_file: File CSV chứa predictions của model (None = auto-detect)
        """
        # Auto-detect project root and set default paths
        ...
        _possible_pred_paths = [
            ...
            _project_root / "multi_label" / "models" / "multilabel_focal" / "test_predictions_detailed.csv",
            _project_root / "BILSTM-STL" / "models" / "test_predictions_detailed.csv",
            _project_root / "VisoBERT-STL" / "models" / "test_predictions_detailed.csv",
        ]
        ...
        if test_file is None:
            ...
            for path in _possible_test_paths:
                ...
                test_file = str(_possible_test_paths[0])  # Default to first if none found
                print(f"  ✗ Not found, using default: {test_file}")
        
        if predictions_file is None:
            print(f"[Path Detection] Checking predictions file locations...")
            for path in _possible_pred_paths:
                print(f"  Checking: {path}")
                if path.exists():
                ...
                predictions_file = str(_possible_pred_paths[0])  # Default to first if none found
                print(f"  ✗ Not found, using default: {predictions_file}")
...
def main():
    """Main function"""
    analyzer = ErrorAnalyzer()  # Auto-detect paths
