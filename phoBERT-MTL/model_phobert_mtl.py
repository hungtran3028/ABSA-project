"""
phoBERT Multi-Task Learning Model for Vietnamese ABSA
======================================================
One shared phoBERT backbone with two task-specific heads:
    - Head 1: Aspect Detection (AD) - Binary classification (11 aspects)
    - Head 2: Sentiment Classification (SC) - 3-class per aspect (11 × 3)

Architecture:
    Input → phoBERT → [CLS] Token
                ↓
    ┌───────────┴───────────┐
    │                       │
  AD Head                SC Head
...
class PhoBERT_MTL(nn.Module):
    """
    phoBERT Multi-Task Learning Model
    
    Shared phoBERT backbone + 2 task-specific heads:
...
def test_model():
    """Test phoBERT MTL model"""
    print("="*80)
    print("Testing phoBERT Multi-Task Learning Model")
    print("="*80)
