"""
phoBERT Multi-Task Learning for Vietnamese ABSA
================================================
Train both AD and SC simultaneously with shared phoBERT backbone
...
# MTL components
from model_phobert_mtl import PhoBERT_MTL
from dataset_phobert_mtl import MTLDataset
...
def setup_logging(output_dir: str) -> str:
    ...
    log_file = os.path.join(output_dir, f'phobert_mtl_log_{timestamp}.txt')
...
def train_epoch_mtl(model, dataloader, optimizer, scheduler, device, 
    ...
    for batch in tqdm(dataloader, desc="[phoBERT-MTL] Training"):
...
def evaluate_mtl(model, dataloader, device, aspect_names):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="[phoBERT-MTL] Evaluating"):
            input_ids = batch['input_ids'].to(device)
            attention_mask = batch['attention_mask'].to(device)
            ad_labels = batch['ad_labels'].to(device)
            sc_labels = batch['sc_labels'].to(device)
...
def save_confusion_matrices(metrics: dict, aspect_names: list, output_dir: str):
    ...
    print("\n[phoBERT-MTL] Generating confusion matrices...")
    ...
    ax.set_ylabel('True')
    ax.set_title('Aspect Detection - Confusion Matrix (phoBERT-MTL)')
    plt.tight_layout()
    ...
    ax.set_title('Sentiment Classification - Overall (10 Aspects, Excluding Others) (phoBERT-MTL)', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def generate_final_report(metrics: dict, output_dir: str, config: dict):
    ...
    report_lines = [
        ...
        f"Model: phoBERT (Pretrained BERT) with 2 Task Heads",
...
def main(args: argparse.Namespace):
    ...
    print("="*80)
    print("PHOBERT MULTI-TASK LEARNING FOR VIETNAMESE ABSA")
    print("="*80)
    ...
    try:
        ...
        wandb.init(
            ...
            name=f"PhoBERT-MTL_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config=config,
            tags=["mtl", "phobert"],
        )
    ...
    # Training loop
    print("\n" + "="*80)
    print("Starting phoBERT-MTL Training")
    print("="*80)
    ...
    for epoch in range(1, num_epochs + 1):
        print(f"\n{'='*80}")
        print(f"[phoBERT-MTL] Epoch {epoch}/{num_epochs}")
        print(f"{'='*80}")
        ...
        logging.info(f"phoBERT-MTL Epoch {epoch}/{num_epochs} - Loss: {train_loss:.4f}")
    ...
    # Test
    print("\n" + "="*80)
    print("[phoBERT-MTL] Testing Best Model")
    print("="*80)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='phoBERT Multi-Task Learning for ABSA')
    parser.add_argument('--config', type=str, default='phoBERT-MTL/config_phobert_mtl.yaml',
