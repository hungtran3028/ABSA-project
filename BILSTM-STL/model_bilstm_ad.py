...
class BiLSTM_AspectDetection(nn.Module):
    ...
    def __init__(
        ...
        if use_phobert_embeddings and phobert_model_name:
            print(f"[BiLSTM_AD] Loading pretrained embeddings from {phobert_model_name}...")
            ...
            print(f"[BiLSTM_AD] ✅ PhoBERT embeddings loaded: {vocab_size} × {embedding_dim} ({freeze_str})")
        else:
            ...
            print(f"[BiLSTM_AD] Using random embeddings: {vocab_size} × {embedding_dim} (TRAINABLE)")
